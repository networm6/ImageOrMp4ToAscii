...
public class CommonUtil {
    ...
    private static final String SD_PATH = Environment.getExternalStorageDirectory().getPath() + "/wudi/";
    ...
    public static Bitmap createAsciiPic(String tt,final String path, Context context) {
	...
	public static Bitmap cr(String it,final Bitmap path, Context context) {
        final String base = it;// 字符串由复杂到简单
        StringBuilder text = new StringBuilder();
        ...
        image = se(path, width1, height1);  //读取图片
        for (int y = 0; y < image.getHeight(); y += 2) {
            for (int x = 0; x < image.getWidth(); x++) {
                final int pixel = image.getPixel(x, y);
                final int r = (pixel & 0xff0000) >> 16, g = (pixel & 0xff00) >> 8, b = pixel & 0xff;
	...
	public static Bitmap se(Bitmap src, int newWidth, int newHeight) {
        Bitmap ret = Bitmap.createScaledBitmap(src, newWidth, newHeight, true);
        return ret;
    }
    public static Bitmap createAsciiPicColor(String it,final String path, Context context) {
        final String base = it;// 字符串由复杂到简单
        StringBuilder text = new StringBuilder();
        List<Integer> colors = new ArrayList<>();
        WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
...
//        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);//绘制透明色
    ...
    public static Bitmap getCompressPhoto(String path) {
        ...
        options.inSampleSize = 70; // 图片的大小设置为原来的十分之一
